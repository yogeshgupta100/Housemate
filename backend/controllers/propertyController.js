...
export const getPropertiesByUser = async (req, res) => {
  try {
    ...
    try {
      ...
      // Get user's active transactions to filter properties
      const { rows: userTransactions } = await client.query(
        `SELECT t.*, p.listing_type as property_listing_type
         FROM transactions t
         JOIN properties p ON t.property_id = p.id
         WHERE t.user_id = $1 
           AND t.is_expired = false
           AND t.status IN ('pending', 'active', 'completed', 'failed')`,
        [userId]
      );

      // Filter properties based on transaction status
      const filteredProperties = properties.filter((property) => {
        ...
        // For rent properties, only show if user has active, non-expired transactions
...
function normalizePropertyData(input, userId) {
  ...
  return {
    ...
    office_area: input.officeArea ? Number(input.officeArea) : null,
    office_floors: input.officeFloors ? Number(input.officeFloors) : null,
    office_capacity: input.officeCapacity ? Number(input.officeCapacity) : null,
    ...
    house_bedrooms: input.houseBedrooms ? Number(input.houseBedrooms) : null,
