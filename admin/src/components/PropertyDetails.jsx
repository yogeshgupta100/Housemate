...
const PropertyDetails = ({ property, onRemove }) => {
  ...
  const handleMapTenant = async () => {
    ...
    if (paymentMethod === "cash" && !cashPaymentAmount) {
      toast.error("Please enter cash payment amount");
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // If cash payment is selected, process it
        if (paymentMethod === "cash") {
          await handleCashPayment(transaction.id);
        }
  ...
  const handleCashPayment = async (transactionId) => {
    try {
      const response = await axios.post(
        `${backendurl}/api/payments/cash-payment`,
        {
          transactionId: transactionId,
          amount: parseFloat(cashPaymentAmount),
          notes:
            cashPaymentNotes ||
            `Cash payment for tenant mapping - ${selectedUser.name}`,
        },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (response.data.success) {
        ...
        throw new Error(
          response.data.message || "Failed to process cash payment"
        );
      }
    } catch (error) {
      console.error("Error processing cash payment:", error);
      toast.error(
        error.response?.data?.message || "Failed to process cash payment"
      );
      throw error; // Re-throw to prevent tenant mapping from completing
    }
  };
  ...
  const handleCheckoutTenant = async (roomId) => {
    try {
      // First, get the active transaction for this room
      ...
      if (transactionResponse.data.success) {
        const activeTransaction =
          transactionResponse.data.activeTransactions.find(
            (transaction) => transaction.roomId === roomId
          );

        if (!activeTransaction) {
          toast.error("No active transaction found for this room");
          return;
        }

        if (
          window.confirm(
            `Are you sure you want to checkout ${activeTransaction.tenant.firstName} ${activeTransaction.tenant.lastName} from Room ${activeTransaction.roomNumber}?`
          )
        ) {
          const checkoutResponse = await axios.post(
            `${backendurl}/api/transactions/admin/checkout/${activeTransaction.transactionId}`,
            {},
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
          ...
          if (checkoutResponse.data.success) {
            ...
            toast.error(
              checkoutResponse.data.message || "Failed to checkout tenant"
            );
          }
        }
      } else {
      ...
      toast.error(error.response?.data?.message || "Failed to checkout tenant");
    }
  };
                              ...
                              {room.occupied ? (
                                <div className="space-y-2">
                                  <button
                                    ...
                                    className="w-full px-3 py-1.5 text-sm bg-yellow-600 text-white rounded hover:bg-yellow-700 transition-colors"
                                  >
                                    Map Tenant
                                  </button>
                                  <button
                                    onClick={() =>
                                      handleCheckoutTenant(room.id)
                                    }
                                    className="w-full px-3 py-1.5 text-sm bg-red-600 text-white rounded hover:bg-red-700 transition-colors"
                                  >
                                    Checkout Tenant
                                  </button>
                                </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-5">
                ...
                <div className="space-y-3">
                  <label className="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-600">
                        Tenant will pay through payment gateway
                      </div>
                    </div>
                  </label>
              ...
              {paymentMethod === "cash" && (
                <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Total amount received in cash (rent + deposit)
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <textarea
                      ...
                      placeholder="Optional notes about the cash payment"
                      rows="3"
                    />
                  </div>
                </div>
              ...
              <div className="flex justify-end space-x-3 pt-4 sticky bottom-0 bg-white z-10">
                ...
                >
                  {paymentMethod === "cash"
                    ? "Map Tenant & Process Cash Payment"
                    : "Map Tenant (Admin)"}
                </button>
              </div>
            </div>
